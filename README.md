...
当前版本：`v0.8.0`（2026-09-01 发布，已在信封/销网/客如云三个真实研究员实例上验证）
...
能力目录现采用执行合同：每项能力必须声明执行模式、入口、输入、输出、依赖、配置字段和质量门。初始化预览会生成依赖有序的执行计划，并明确显示尚未安装的适配器；实例配置只保存精简清单，不复制完整能力定义。
...
来源根目录包含管理说明或自动生成清单时，可在实例配置中设置
`pipeline.source_exclude_globs`，以相对路径 glob 排除非来源文件；发现与来源审计使用同一规则。处理用户明确指定的单篇来源时，可把发现结果中的 `job_id` 同时传给 `extract --job-id` 和 `claim --job-id`，避免提取或领取其他未读来源。
Markdown 原文已有作者、账号、发布时间、保存时间和 URL 时，提交阶段会用这些 frontmatter 字段补齐模型未提供的来源归属；模型显式提供的值优先。
...
这个项目的当前版本是 `v0.8.0`，可以作为 Beta 版使用：
...
本项目使用语义版本号。GitHub 当前公开版本为 `v0.8.0`。`v0.8.0` 保持旧 CLI 和配置兼容，并增加多维研究员、能力执行合同、模型事务账本、引用检索、适配器诊断和企业隐私边界；同时修复了非ASCII研究员名称的运行时命名空间碰撞问题，新增 `quality.essential_aliases`/`quality.require_theme_cluster_at_refinement` 两个配置项。
