...
- Use `references/privacy-and-enterprise-connectors.md` whenever adding MCP, 钉钉、飞书 or other enterprise integrations. Shared packages may declare connector contracts, but instance configuration, credentials and enterprise knowledge must remain researcher-local.
- Use `references/capability-contracts.md` whenever adding or changing a researcher capability, dependency, adapter, execution entrypoint, input/output type, or capability quality gate.
