...
共享包采用一个模块化单体内核。每个研究员实例选择一个面向任务的类型，再解析为依赖有序的能力执行合同。合同区分内置确定性程序、模型指令和实例适配器，并声明可移植的输入、输出、入口、配置字段名和质量门。
...
旧配置和 `knowledge|video` profile 保持兼容。新增研究员类型和能力必须扩展共享合同，不能复制内核。
